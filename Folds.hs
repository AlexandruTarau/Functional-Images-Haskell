...
listExceptLast [] = []
listExceptLast [_] = []
listExceptLast (x:xs) = x : listExceptLast xs
...
instance Show TransformationAST where
    ...
    show (T (Combine transformations)) =
      let transformations_except_last = listExceptLast transformations
        in "[" ++ foldr (\x acc -> show x ++ "," ++ acc) "" transformations_except_last
               ++ (show $ last transformations) ++ "]"
...
instance Show RegionAST where
    show r = showAST r 0
      where
        showAST :: RegionAST -> Int -> String
        showAST (R (FromPoints points)) n = spaces n ++ "FromPoints " ++ show points ++ "\n"
        showAST (R (Rectangle x y)) n = spaces n ++ "Rectangle " ++ show x ++ " " ++ show y ++ "\n"
        showAST (R (Circle r)) n = spaces n ++ "Circle " ++ show r ++ "\n"
        showAST (R (Transform t r)) n = spaces n ++ show t ++ "\n" ++ showAST r (n + 1)
        showAST (R (Complement r)) n = spaces n ++ "~" ++ "\n" ++ showAST r (n + 1)
        showAST (R (Union r1 r2)) n = spaces n ++ "+" ++ "\n" ++ showAST r1 (n + 1) ++ showAST r2 (n + 1)
        showAST (R (Intersection r1 r2)) n = spaces n ++ "*" ++ "\n" ++ showAST r1 (n + 1) ++ showAST r2 (n + 1)
...
instance Show RegionAST where
    show r = removeNewLine $ foldRegionAST combiner r 0
      where
        combiner :: RegionCombiner (Int -> String)
        combiner (FromPoints points) = \n -> spaces n ++ "FromPoints " ++ show points ++ "\n"
        combiner (Rectangle x y) = \n -> spaces n ++ "Rectangle " ++ show x ++ " " ++ show y ++ "\n"
        combiner (Circle r) = \n -> spaces n ++ "Circle " ++ show r ++ "\n"
        combiner (Transform t subRegion) = \n -> spaces n ++ show t ++ "\n" ++ subRegion (n + 1)
        combiner (Complement subRegion) = \n -> spaces n ++ "~\n" ++ subRegion (n + 1)
        combiner (Union r1 r2) = \n -> spaces n ++ "+\n" ++ r1 (n + 1) ++ r2 (n + 1)
        combiner (Intersection r1 r2) = \n -> spaces n ++ "*\n" ++ r1 (n + 1) ++ r2 (n + 1)
...
instance Num RegionAST where
    fromInteger n = R $ FromPoints [(fromIntegral n :: Float, fromIntegral n :: Float)]
...
instance Functor TransformationShape where
    ...
    fmap f (Scaling n) = Scaling n
    fmap f (Translation x y) = Translation x y
...
instance Functor RegionShape where
    ...
    fmap f (Circle r) = Circle r
    fmap f (Rectangle x y) = Rectangle x y
    fmap f (FromPoints points) = FromPoints points
    fmap f (Complement r) = Complement $ f r
    fmap f (Union r1 r2) = Union (f r1) (f r2)
    fmap f (Intersection r1 r2) = Intersection (f r1) (f r2)
    fmap f (Transform transformation r) = Transform transformation $ f r
...
toTransformation = foldTransformationAST combiner
  where
    ...
    combiner (Scaling n) = S.scaling n
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner (Combine transformations) = foldr (+) 0 transformations
    combiner transformation = 1
...
{-
    *** TODO ***
    ...
    Răspuns:
    Evaluarea lenesa poate ajuta in situatia in care dorim sa accesam doar
    o componenta a unei perechi (tuple) dintr-o structura de date, cum ar fi o lista
    sau un arbore, deoarece ne permite sa evitam evaluarea componentei pe care nu o folosim.
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Union r1 r2) = (fst r1 + fst r2, snd r1 + snd r2)
    combiner (Transform t r) = (fst r, snd r + basicTransformationCount t)
    combiner (Complement r) = r
    combiner (Intersection r1 r2) = (fst r1 + fst r2, snd r1 + snd r2)
    combiner region = (1, 0)
