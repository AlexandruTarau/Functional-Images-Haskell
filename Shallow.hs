...
plot width height region =
    let string = map (\y ->
                    ...
                    [fromIntegral x | x <- [-width .. width]])
                [fromIntegral y | y <- [-height .. height]]
    in intercalate "\n" string
...
bfs start expand = [(start, 0)] ++ bfs_rec [(start, 0)] (S.singleton start) expand


bfs_rec states visited expand = if null resultList
                                then []
                                else (concat resultList) ++ (bfs_rec (concat resultList) resultVisited expand)
    where
        (resultVisited, resultList) =  mapAccumL bfsStep visited states

        bfsStep visited (state, dist) =
            let newStates = filter (`S.notMember` visited) (expand state)
                newStateDistPairs = zip newStates (repeat (dist + 1))
                newVisited = S.union visited (S.fromList newStates)
            in (newVisited, newStateDistPairs)
...
regionAvoidingBfs start region = bfs start my_expand
    where
        my_expand point = 
            [p | p <- [p1, p2, p3, p4], not $ inside p region]
            where
                p1 = translation 1 0 point
                p2 = translation (-1) 0 point
                p3 = translation 0 1 point
                p4 = translation 0 (-1) point
